...
import { checkinByCode, queryCheckinByCode, type CheckinResult } from '@/api/checkin'
...
let scannerId = 'checkin-qr-reader'
...
const handleQuery = async () => {
  ...
  loading.value = true
  try {
    const res = await queryCheckinByCode(c)
...
const handleCheckin = async () => {
  ...
  try {
    ...
    const res = await checkinByCode(c)
    detail.value = res.data
    ...
    <div class="checkin-card">
      ...
      <div class="input-wrap">
        ...
        <el-input
          ...
          maxlength="6"
          input-style="font-size: 22px; text-align: center; letter-spacing: 6px;"
          class="code-input"
          type="number"
          pattern="\\d*"
        />
      ...
      <div v-if="detail" class="detail-card">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ detail.actualFee != null ? `¥${detail.actualFee}` : '-' }}</span>
        </div>
