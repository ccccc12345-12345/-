...
const load = async () => {
  loading.value = true
  try {
    const res = await getStaffList(keyword.value || undefined)
    list.value = res.data || []
  } finally {
    loading.value = false
  }
...
const save = async () => {
  ...
  if (isEdit.value && editId.value) {
    await updateStaff(editId.value, data)
    ElMessage.success('更新成功')
  } else {
    await createStaff({ ...data, password: form.password })
...
const toggleStatus = async (row: Staff) => {
  ...
  try {
    ...
    await updateStaffStatus(row.id, nextStatus)
    ElMessage.success(`${actionText}成功`)
    load()
  } catch {}
}

const openResetPassword = (row: Staff) => {
  resetPwdForm.staffId = row.id
  resetPwdForm.staffName = row.staffName
  resetPwdForm.newPassword = ''
  resetPwdDialogVisible.value = true
  setTimeout(() => resetPwdFormRef.value?.clearValidate(), 0)
}
...
const confirmResetPassword = async () => {
  ...
  if (!valid) return
  try {
    const res = await resetStaffPassword(resetPwdForm.staffId, resetPwdForm.newPassword)
    ElMessageBox.alert(`员工“${resetPwdForm.staffName}”的新密码为：${res.data}`, '重置成功', { type: 'success' })
    resetPwdDialogVisible.value = false
  } catch {}
}

const del = async (row: Staff) => {
  try {
      ...
      <el-table :data="list" stripe>
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link :type="row.status === 'normal' ? 'danger' : 'success'" @click="toggleStatus(row)">
    ...
    <el-dialog v-model="resetPwdDialogVisible" title="重置密码" width="400px">
      <el-form ref="resetPwdFormRef" :model="resetPwdForm" :rules="resetPwdRules" label-width="90px">
        <el-form-item label="员工">
          <span>{{ resetPwdForm.staffName }}</span>
        </el-form-item>
