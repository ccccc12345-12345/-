...
const form = reactive<Partial<RestaurantMenu>>({
...
const numberRequired = (message: string) => (rule: any, value: any, callback: any) => {
  if (value === undefined || value === null || value === '') {
    callback(new Error(message))
  } else if (Number(value) < 0) {
    callback(new Error(`${message.replace('请输入', '')}不能小于0`))
  } else {
    callback()
  }
}
...
const rules = {
  ...
  price: [{ validator: numberRequired('请输入价格'), trigger: 'blur' }]
}
...
const load = async () => {
  loading.value = true
  try {
    const res = await getMerchantRestaurantMenus()
...
const save = async () => {
  ...
  if (!valid) return
  const data: Partial<RestaurantMenu> = {
    pondId: form.pondId,
    name: form.name.trim(),
