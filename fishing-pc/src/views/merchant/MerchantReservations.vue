...
const merchantPondId = computed(() => {
  const id = Number(userStore.pondId)
...
const loadSlots = async () => {
  try {
    const res = await getMerchantTimeSlots({ pondId: merchantPondId.value, pageSize: 100 })
    ...
    <el-card class="filter-card">
      <el-form :model="query" inline>
        <el-form-item label="鱼塘">
          <el-select v-model="query.pondId" placeholder="选择鱼塘" clearable style="width: 160px" @change="query.slotId = undefined; query.pageNum = 1; loadSlots(); load()">
            <el-option
              v-for="pond in ponds"
              :key="pond.id"
              :label="pond.name"
        ...
        <el-form-item>
          ...
          <el-button icon="Refresh" @click="query.slotDate = ''; query.slotId = undefined; query.status = ''; query.pageNum = 1; query.pondId = ponds[0]?.id; loadSlots(); load()">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card v-loading="loading">
      <el-table :data="list" stripe>
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/user/reservations/${row.id}`)">详情</el-button>
            <el-button v-if="row.status === '待抽号'" link type="danger" @click="cancel(row)">取消</el-button>
            <el-button v-if="row.status === '已抽号' && row.checkinCode" link type="success" @click="$router.push(`/merchant/checkin?code=${row.checkinCode}`)">去核销</el-button>
          </template>
        </el-table-column>
      </el-table>
